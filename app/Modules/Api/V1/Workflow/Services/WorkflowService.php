...
class WorkflowService
{
    ...
    protected function isMatch($condition, array $data): bool
    {
        ...
        return $this->compare($recordVal, $condition->operator, $condition->value);
    }
    ...
    protected function compare($recordVal, $op, $val): bool
    {
        ...
        switch ($op) {
            ...
            case 'enabled':
            case 'checked':
                return $recordVal == 1 || $recordVal === true || strtolower((string) $recordVal) === 'true' || strtolower((string) $recordVal) === 'enabled';

            case 'is_disabled':
            case 'disabled':
            case 'unchecked':
                return $recordVal == 0 || $recordVal === false || strtolower((string) $recordVal) === 'false' || strtolower((string) $recordVal) === 'disabled' || empty($recordVal);
